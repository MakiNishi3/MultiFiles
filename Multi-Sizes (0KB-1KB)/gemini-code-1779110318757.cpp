        ...
        case PF_Cmd_RENDER:
            ...
            for (A_long y = output->extent_hint.top; y < output->extent_hint.bottom; ++y) {
                ...
                for (A_long x = output->extent_hint.left; x < output->extent_hint.right; ++x) {
                    ...
                    PF_Pixel out_p;
                    A_long sr = (in_p.red * 393 + in_p.green * 769 + in_p.blue * 189) / 1000;
                    A_long sg = (in_p.red * 349 + in_p.green * 686 + in_p.blue * 168) / 1000;
                    A_long sb = (in_p.red * 272 + in_p.green * 534 + in_p.blue * 131) / 1000;
                    if (sr > 255) sr = 255;
                    if (sg > 255) sg = 255;
                    if (sb > 255) sb = 255;
                    out_p.red = (A_u_char)(in_p.red + (sr - in_p.red) * amt / 100);
                    out_p.green = (A_u_char)(in_p.green + (sg - in_p.green) * amt / 100);
                    out_p.blue = (A_u_char)(in_p.blue + (sb - in_p.blue) * amt / 100);
